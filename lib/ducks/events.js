...
const TITLE_UPDATE_CHANGED_ACTION = "TITLE_UPDATE_CHANGED_ACTION";
const DESCRIPTION_UPDATE_CHANGED_ACTION = "DESCRIPTION_UPDATE_CHANGED_ACTION";
const DATE_UPDATE_CHANGED_ACTION = "DATE_UPDATE_CHANGED_ACTION";
const TIME_UPDATE_CHANGED_ACTION = "TIME_UPDATE_CHANGED_ACTION";
const CAPACITY_UPDATE_CHANGED_ACTION = "CAPACITY_UPDATE_CHANGED_ACTION";
...
export default function reducer(state = initialState, action) {
	...
	switch (action[TYPE_KEY]) {
		...
		case CAPACITY_CHANGED_ACTION:
			...
			break;
		case TITLE_UPDATE_CHANGED_ACTION:
			newState = {
				...state,
				[EVENT_DETAIL]: {
					...state[EVENT_DETAIL],
			...
			break;
		case DESCRIPTION_UPDATE_CHANGED_ACTION:
			newState = {
				...state,
				[EVENT_DETAIL]: {
					...state[EVENT_DETAIL],
			...
			break;
		case DATE_UPDATE_CHANGED_ACTION:
			newState = {
				...state,
				[EVENT_DETAIL]: {
					...state[EVENT_DETAIL],
			...
			break;
		case TIME_UPDATE_CHANGED_ACTION:
			newState = Object.assign({}, state, {
				[TIME]: action[TIME]
			});
			break;
		case CAPACITY_UPDATE_CHANGED_ACTION:
			newState = {
				...state,
				[EVENT_DETAIL]: {
					...state[EVENT_DETAIL],
...
export function updateEvent(eventId) {
	return (dispatch, getState) => {
		...
		eventId ? rest.doPatch(
			...
			dispatch(recieveEventSuccessful(createdEvent));
			dispatch(closeCreateDialog(false));
			history.push("events");
			dispatch(infoMessage(`You have successufuly updated event ${createdEvent.title}`))
		}, reason => {
			printInfo(dispatch, JSON.parse(reason.responseText).errors);
			dispatch(messagesReceived(createErrorMessages(reason)));
		}) : null;
	}
}
...
export function titleUpdateChanged(title) {
	return {
		[TYPE_KEY]: TITLE_UPDATE_CHANGED_ACTION,
		[TITLE]: title
	};
}

export function descriptionUpdateChanged(description) {
	return {
		[TYPE_KEY]: DESCRIPTION_UPDATE_CHANGED_ACTION,
		[DESCRIPTION]: description
	};
}

export function dateUpdateChanged(date) {
	return {
		[TYPE_KEY]: DATE_CHANGED_ACTION,
		[DATE]: date
	};
...
export function timeUpdateChanged(date) {
	return {
		[TYPE_KEY]: TIME_UPDATE_CHANGED_ACTION,
		[TIME]: date
	};
}

export function capacityUpdateChanged(capacity) {
	return {
		[TYPE_KEY]: CAPACITY_UPDATE_CHANGED_ACTION,
		[CAPACITY]: capacity
	};
}
...
function printInfo(dispatch, obj) {
	Object.keys(obj).forEach((key) => {
		dispatch(infoMessage(obj[key].message));
	});
}
