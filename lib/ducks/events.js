...
export const UPDATED_EVENT = "eventToUpdate";
...
export default function reducer(state = initialState, action) {
	...
	switch (action[TYPE_KEY]) {
		...
		case SUCCESSFULLY_ATTENDED:
			newState = Object.assign({}, state, {
				[UPDATED_EVENT]: action[UPDATED_EVENT],
		...
		case NEW_EVENT_SUCCESSFULLY_CREATED:
			newState = Object.assign({}, state, {
				[NEW_CREATED_EVENT]: action[NEW_CREATED_EVENT]
			});
			break;
		default:
			newState = Object.assign({}, state, {});
...
export function createEvent() {
	return (dispatch, getState) => {
		let data = {
			...
			startsAt: getState().events.date.toISOString(),
		...
		rest.doPost(
			...
			try {
				...
				let newEvent = Object.assign(event, createdEvent);
				let updatedList = Object.assign(getState().events.events, newEvent);
				dispatch(recieveEventListSuccessful(newEvent, true, false, false));
			} catch (e) {
				dispatch(messagesReceived(createErrorMessages(e)));
			}
		}, reason => {
	...
	return (dispatch, getState) => {
		...
		eventId && token ? rest.doPost(
			...
			try {
				let oldEvent = getState().events.events.filter(event => event.id === eventId)[0];
				let newEvent = Object.assign(oldEvent, attendedEvent);
				let listUpdate = Object.assign(getState().events.events, newEvent);
				dispatch(recieveEventListSuccessful(listUpdate, true, false, false));
			} catch (e) {
				dispatch(messagesReceived(createErrorMessages(e)));
			}
		}, reason => {
